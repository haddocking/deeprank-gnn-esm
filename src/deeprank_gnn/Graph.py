...
class Graph(object):
    ...
    def h52nx(self, f5name, mol, molgrp=None):
        ...
        # get internal edges
        edges = molgrp["internal_edges"][()].astype("U").tolist()
        edges = [(tuple(e[0]), tuple(e[1])) for e in edges]
        ...
        edge_feat = {}
        for key in edge_key:
            edge_feat[key] = molgrp["internal_edge_data/" + key][()]
        ...
        if "clustering" in molgrp:
            ...
            for method in list(molgrp["clustering"].keys()):
                self.clusters[method] = molgrp["clustering/" + method + "/depth_0"][()]

        if molgrp is None:
            f5.close()

    def plotly_2d(
        self, out=None, offline=False, iplot=True, disable_plot=False, method="louvain"
    ):
        ...
        try:
            ...
            print(
                "No cluster in the group. Computing cluster now with method : %s"
                % method
            )
